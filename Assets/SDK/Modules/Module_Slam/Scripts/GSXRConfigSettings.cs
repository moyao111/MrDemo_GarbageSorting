...
public class GSXRConfigSettings : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        {
            DisableSvrInput();
            SetSvrCameraView(GSXRConfigOptions.Instance.FixedViewportPosition, GSXRConfigOptions.Instance.FixedViewportEulerAnglesRotation);
        }

        if (GSXRConfigOptions.Instance.OverrideRenderTextureMSAA != 0)
        {
            SetSvrRenderTextureAntialiasing(GSXRConfigOptions.Instance.OverrideRenderTextureMSAA);
        }
