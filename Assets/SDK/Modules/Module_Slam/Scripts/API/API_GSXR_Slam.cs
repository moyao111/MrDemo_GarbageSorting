...
public class API_GSXR_Slam
{
    ...
    {
        API_GSXR_Slam.SlamManager.SvrInitializedCallBack += action;
    }
    ...
    public static void GSXR_Set_RenderFrame(int frameRate = -1)
    {
        if (API_GSXR_Slam.SlamManager != null)
        {
            if (frameRate == -1)
            {
                API_GSXR_Slam.SlamManager.plugin.SetVSyncCount((int)(API_GSXR_Slam.SlamManager.settings.vSyncCount = GSXRManager.SvrSettings.eVSyncCount.k1));
                QualitySettings.vSyncCount = (int)(API_GSXR_Slam.SlamManager.settings.vSyncCount = GSXRManager.SvrSettings.eVSyncCount.k1);//Vsync
            }
            else
            {
                API_GSXR_Slam.SlamManager.plugin.SetVSyncCount((int)(API_GSXR_Slam.SlamManager.settings.vSyncCount = GSXRManager.SvrSettings.eVSyncCount.k0));
                QualitySettings.vSyncCount = (int)(API_GSXR_Slam.SlamManager.settings.vSyncCount = GSXRManager.SvrSettings.eVSyncCount.k0);//Don't sync
    ...
    public static int GSXR_Get_LatestQVRCameraBinocularData(ref bool outBUdate, ref uint outCurrFrameIndex, ref ulong outFrameExposureNano, byte[] outLeftFrameData, byte[] outRightFrameData)
    {
        if (API_GSXR_Slam.SlamManager && API_GSXR_Slam.SlamManager.plugin != null)
        {
            ...
            return API_GSXR_Slam.SlamManager.plugin.GSXR_Get_LatestQVRCameraBinocularData(ref outBUdate, ref outCurrFrameIndex, ref outFrameExposureNano, outLeftFrameData, outRightFrameData);
        }
        return 0;
    }
