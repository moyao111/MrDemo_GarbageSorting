...
class SvrPluginAndroid : GSXRPlugin
{

    private const string DLLName = "svrplugin";
    ...
    [DllImport(DLLName)]
	private static extern void SvrControllerGetState(int handle, int space, ref GSXRControllerState state);
    ...
    public override void SubmitFrame(int frameIndex, float fieldOfView, int frameType)
	{
        ...
        if (eyes != null) for (i = 0; i < eyes.Length; i++)
        {
            ...
            SvrSetupLayerData(layerCount, (int)eye.Side, eye.TextureId, eye.ImageType == GSXREye.eType.EglTexture ? 2 : 0, eye.layerDepth > 0 ? 0x0 : 0x2);
        ...
        if (overlays != null) for (i = 0; i < overlays.Length; i++)
        {
            ...
            SvrSetupLayerData(layerCount, (int)overlay.Side, overlay.TextureId, overlay.ImageType == GSXROverlay.eType.EglTexture ? 2 : 0, 0x1);
    ...
    public override bool PollEvent(ref GSXRManager.SvrEvent frameEvent)
    {
		...
		frameEvent.eventType = (GSXRManager.svrEventType)(eventType);
        switch (frameEvent.eventType)
        {
            case GSXRManager.svrEventType.kEventThermal:
                //Debug.LogFormat("PollEvent: data {0} {1}", dataBuffer[0], dataBuffer[1]);
                frameEvent.eventData.thermal.zone = (GSXRManager.svrThermalZone)dataBuffer[0];
                frameEvent.eventData.thermal.level = (GSXRManager.svrThermalLevel)dataBuffer[1];
                break;
        }
        return isEvent;
    }
	...
	//---------------------------------------------------------------------------------------------
	public override void ControllerSendMessage(int handle, GSXRController.svrControllerMessageType what, int arg1, int arg2)
	{
	...
	//---------------------------------------------------------------------------------------------
	public override object ControllerQuery(int handle, GSXRController.svrControllerQueryType what)
	{
		int memorySize = 0;
		...
		switch(what)
		{
			case GSXRController.svrControllerQueryType.kControllerBatteryRemaining:
				{
				...
				break;
			case GSXRController.svrControllerQueryType.kControllerControllerCaps:
				{
    ...
    public override int GSXR_Get_ControllerMode() {return 6;}

    public override int GSXR_Get_ControllerNum() {return 2;}

    public override void GSXR_Set_ControllerKeyEventCallback(OnKeyEvent _event) { ScHandShank_SetKeyEventCallback(_event); }

    public override void GSXR_Set_ControllerKeyTouchEventCallback(OnKeyTouchEvent _event) { ScHandShank_SetKeyTouchEventCallback(_event); }

    public override void GSXR_Set_ControllerRockerCallback(OnTouchEvent _event) { ScHandShank_SetTouchEventCallback(_event); }

    public override void GSXR_Set_ControllerHallCallback(OnHallEvent _event) { ScHandShank_SetHallEventCallback(_event); }

    public override void GSXR_Set_ControllerChargingEventCallback(OnChargingEvent _event) { ScHandShank_SetChargingEventCallback(_event); }

    public override void GSXR_Set_ControllerBatteryEventCallback(OnBatteryEvent _event) { ScHandShank_SetBatteryEventCallback(_event); }

    public override void GSXR_Set_ControllerConnectEventCallback(OnConnectEvent _event) { ScHandShank_SetConnectEventCallback(_event); }


    public override int GSXR_Get_ControllerBattery(int lr) { return ScHandShank_GetBattery(lr); }

    public override int GSXR_Get_ControllerVersion(int lr) { return ScHandShank_Getbond(lr); }

    public override int GSXR_Get_ControllerList(int lr) { return ScHandShank_Getbond(lr); }

    public override bool GSXR_Is_ControllerConnect(int lr) { return ScHandShank_GetConnectState(lr); }
    ...
    #region HandTracking
    [DllImport(DLLName)]
    protected static extern void ScHANDTRACK_Start(Action<int> func);

    [DllImport(DLLName)]
    public static extern void ScHANDTRACK_Stop();

    [DllImport(DLLName)]
    private static extern void ScHANDTRACK_SetGestureCallback(Action<int> func);
    ...
    [DllImport(DLLName)]
    private static extern int ScHANDTRACK_SetGestureModelDataCallback(Action func);

    [DllImport(DLLName)]
    protected static extern void ScHANDTRACK_SetLowPowerWarningCallback(Action<int> func);

    public override bool GSXR_Is_SupportHandTracking() {
        return true;
    }
    public override void GSXR_StartHandTracking(Action<int> func) {
        ScHANDTRACK_Start(func);
    }
    ...
    public override void GSXR_Get_HandTrackingData(float[] mode, float[] pose) {
        ScHANDTRACK_GetGesture(mode,  pose);
    }
    ...
    public override int GSXR_Set_HandTrackingData(float[] mode, float[] pose) {
        return ScHANDTRACK_SetGestureData(mode, pose);
    }
    public override int GSXR_Set_HandTrackingModelDataCallBack(Action callback) {
        return ScHANDTRACK_SetGestureModelDataCallback(callback);
    }
    ...
    public override void GSXR_Get_LatestQVRCameraFrameData(ref bool outBUdate, ref uint outCurrFrameIndex, ref ulong outFrameExposureNano,
                byte[] outFrameData, float[] outTRDataArray) {
    ...
    public override void GSXR_Get_LatestQVRCameraFrameDataNoTransform(ref bool outBUdate, ref uint outCurrFrameIndex, ref ulong outFrameExposureNano,
            byte[] outFrameData, float[] outTRDataArray) {
    ...
    public override int GSXR_Get_LatestQVRCameraBinocularData(ref bool outBUdate, ref uint outCurrFrameIndex, ref ulong outFrameExposureNano, byte[] outLeftFrameData, byte[] outRightFrameData) {
    ...
    [DllImport(DLLName)]
    public static extern void Sc_setAppExitCallback(Action callBack);
    public override void SetGlassDisconnectedCallBack(Action callBack) {
        if (API_Module_Device.CurrentAndroid.type == SC.XR.Unity.Module_Device.AndroidDeviceType.HONGHU) {
            Sc_setAppExitCallback(callBack);
        } else if (API_Module_Device.CurrentAndroid.type == SC.XR.Unity.Module_Device.AndroidDeviceType.MV201) {
