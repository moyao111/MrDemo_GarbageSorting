    ...
    public class InputDataGetKSPosture : InputDataGetGCPosture {
        ...
        void UpdateTransform() {
            ...
            if(Application.platform != RuntimePlatform.Android) {
                if(API_GSXR_Slam.SlamManager != null && API_GSXR_Slam.SlamManager.gameObject.activeSelf) {
                    inputDataGetKS.inputDeviceKSPart.inputDataKS.rotation = API_GSXR_Slam.SlamManager.head.rotation;
                }

                if(inputDataGetKS.inputDeviceKSPart.PartType == InputDevicePartType.KSLeft) {
                    if(API_GSXR_Slam.SlamManager != null && API_GSXR_Slam.SlamManager.gameObject.activeSelf) {
                        inputDataGetKS.inputDeviceKSPart.inputDataKS.position = API_GSXR_Slam.SlamManager.head.position + leftPositionDeltaWithHead;
                    }
                } else if(inputDataGetKS.inputDeviceKSPart.PartType == InputDevicePartType.KSRight) {
                    if(API_GSXR_Slam.SlamManager != null && API_GSXR_Slam.SlamManager.gameObject.activeSelf) {
                        inputDataGetKS.inputDeviceKSPart.inputDataKS.position = API_GSXR_Slam.SlamManager.head.position + rightPositionDeltaWithHead;
                    }
                } else {
            ...
            if (API_GSXR_Slam.SlamManager.specificationType == GSXRManager.SpecificationType.GSXR) {
                ...
                if (postureType == PostureType._6Dof) {
                    ...
                    if (inputDataGetKS.inputDeviceKSPart.PartType == InputDevicePartType.KSLeft) {
                        result = API_GSXR_Slam.SlamManager.plugin.Fetch6dofHandShank(array, 0);
                    } else if (inputDataGetKS.inputDeviceKSPart.PartType == InputDevicePartType.KSRight) {
                        result = API_GSXR_Slam.SlamManager.plugin.Fetch6dofHandShank(array, 1);
                    }
                    ...
                    //Rotation
                    if (inputDataGetKS.inputDeviceKSPart.PartType == InputDevicePartType.KSLeft) {
                        result = API_GSXR_Slam.SlamManager.plugin.Fetch3dofHandShank(array, 0);
                    } else if (inputDataGetKS.inputDeviceKSPart.PartType == InputDevicePartType.KSRight) {
                        result = API_GSXR_Slam.SlamManager.plugin.Fetch3dofHandShank(array, 1);
                    }
        ...
        void Update3DofDeltaEulerAngles() {
            if(API_GSXR_Slam.SlamManager && API_GSXR_Slam.SlamManager.gameObject.activeSelf) {
                deltaEulerAngles = API_GSXR_Slam.SlamManager.head.transform.localEulerAngles - rotation.eulerAngles;
